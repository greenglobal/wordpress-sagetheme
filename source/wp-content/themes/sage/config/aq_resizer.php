...
if(!function_exists('aq_resize')) {
    ...
    function get_image_featured($post_id,$width=0,$height=0){
        if(has_post_thumbnail($post_id)){
            $thumb   = get_post_thumbnail_id($post_id);
            $img_url = wp_get_attachment_url( $thumb,'full');
            $img   = aq_resize($img_url,$width,$height, true );
            if(!$img){
                $img   = aq_resize( $img_url,$width,$height, false );
            }

        }else{
            $img = get_template_directory_uri()."/assets/images/not-found.png";
        }
        return $img;
    }
    /**
     * get_image_featured url
     */
    function get_image_featured_uri($img_url,$width=0,$height=0){
        if(!empty($img_url)){
            $img   = aq_resize( $img_url,$width,$height, true );
            if(!$img){
                $img   = aq_resize( $img_url,$width,$height, false );
